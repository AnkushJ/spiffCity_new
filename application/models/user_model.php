 ...
 */
	...
	class User_model extends CI_Model{
		...
		function save_fb_user_details($arrData){
			if($this->db->insert('sp_users', $arrData)){
				return true;
			}
			else{
		...
		*/
		...
		function delete_user($id){
			if($this->db->delete('sp_users', array('id' => $id))){
				return true;
			}
			else{
				return false;
		...
		function update_user($iUserId,$arrData){
			$this->db->where('id',$iUserId);
			if($this->db->update('sp_users', $arrData)){
				return true;
			}
			else{
				return false;
